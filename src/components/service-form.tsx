...
import { getManagedMiddlewareNames, getUnknownMiddlewareNames, parseMiddlewareNames } from "@/lib/middleware-utils";
...
export function ServiceForm({
  ...
  const unknownEntrypoint = canValidateEntrypoints && formData.entrypoint
    ...
    ? formData.entrypoint
    : null;
  const managedMiddlewareNames = getManagedMiddlewareNames(formData.managedMiddlewares);
  const knownMiddlewareNames = [
    ...availableMiddlewares.map((middleware) => middleware.name),
    ...managedMiddlewareNames,
  ];
  const unknownMiddlewares = canValidateMiddlewares
    ? getUnknownMiddlewareNames(middlewareText, knownMiddlewareNames)
    : [];
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                {unknownMiddlewares.length > 0 && (
                  <div className="flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800 dark:border-amber-800 dark:bg-amber-950/20 dark:text-amber-300">
                    ...
                    <div>
                      <p className="font-medium">Not found in discovered or app-managed Traefik middlewares:</p>
              ...
              <div className="space-y-2">
                  ...
                  <HelpLabel
                    help={
                      <ContextHelp title="Managed middleware JSON" href="/docs/services#managed-middlewares-json">
                        ...
                        <p>Defining a middleware here only creates it; it is not applied until its name is also added to Middlewares above.</p>
                      </ContextHelp>
                    }
                  >
                ...
                />
                <p className="text-xs text-gray-500">
                  Creates Traefik middleware definitions only. To apply one, add its name to Middlewares (comma-separated) above, for example redirect-to-admin.
                </p>
              </div>
