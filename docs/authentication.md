...
The Sessions page also records lightweight request context for abuse review: auth method, client IP and header source, last IP, user agent, requested host, entrypoint, last path, access count, and derived risk flags such as IP or user-agent changes. The inventory can filter normal user sessions separately from observed bypass rows, and can narrow further to SSO or shared-link sessions. Avoid storing sensitive request details such as full URLs with query strings, cookies, tokens, raw headers, or request bodies in session metadata.
...
When service SSO runs behind Traefik, configure **Public TPA URL for Browser/OAuth** on the Config page to TPA's public HTTPS hostname. The generated forwardAuth address can use **Internal TPA URL for Traefik**, but browser redirects for `/api/auth/sso/login` and OAuth callbacks must use the public URL. The public TPA URL can protect services on multiple domains because each service domain is carried in signed state and receives its own host-scoped session cookie after ticket redemption.
